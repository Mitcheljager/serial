...
<style lang="scss">
  @keyframes active-home {
    0% { transform: scale(1) }
    25% { transform: scale(1, .5) translateY(-25%) rotate(10deg) }
    50% { transform: scale(1, 1.1) translateY(0) rotate(5deg) }
    75% { transform: scale(1, .75) rotate(-5deg) }
    100% { transform: scale(1) }
  }

  svg {
    &.active {
      animation: active-home 500ms;
